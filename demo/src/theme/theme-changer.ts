...
export class ThemeChanger extends LitElement {
  ...
  render() {
    return html`
      ...
      ${this.renderHexPicker()} ${this.renderHctPicker()} ${this.renderColorModePicker()}
    `;
  }
    ...
    return html`<div>
      <label id="hex" for="color-input">
        ...
        <span class="input-wrapper">
          <div class="overflow">
            <input id="color-input" @input=${this.onHexPickerInput} type="color" .value=${live(this.hexColor)} />
          </div>
  ...
  private renderHctPicker() {
    return html`<div class="sliders">
      <hct-slider .value=${live(this.hue)} type="hue" label="Hue" max="360" @input=${this.onSliderInput}></hct-slider>
      <hct-slider
  ...
  private renderColorModePicker() {
    ...
    >
      ${this.renderModeButton('dark', 'dark_mode')} ${this.renderModeButton('auto', 'brightness_medium')}
  ...
  static styles = css`
    ...
    copy-code-button {
       ...
       */
      --_inline-block-inset: calc(-1 * (var(--_copy-button-button-size) - var(--_copy-button-icon-size)) / 2);
