  ...
  if (relPathFromPkgRoot.indexOf('../') === 0) {
    error(
      `Import source '${source}' is outside of the package root '${pkgRoot}'. ` +
        'Perhaps this should be a module import instead?',
      source,
      srcFile,
      node,
    );
...
function assertNoLocalDirectoryImports(source, pkgRoot, srcFile, node) {
  if (source.indexOf(packagePrefix) === -1 && source[0] !== '.') {
    return; // skip check for external dependencies
  }

  const srcDir = path.dirname(srcFile);
  const resolvedSrc = path.relative(process.cwd(), path.resolve(srcDir, source));
  const relPathFromPkgRoot =
    resolvedSrc.indexOf(packagePrefix) > -1
      ? './packages/mdc-' + resolvedSrc.slice(resolvedSrc.indexOf(packagePrefix) + 10)
      : './' + resolvedSrc;

  if (
    !fs.existsSync(path.extname(relPathFromPkgRoot) ? relPathFromPkgRoot : relPathFromPkgRoot + '.js') &&
...
function assertModuleImportSpecifiedAsPkgDep(source, pkgRoot, srcFile, node) {
  ...
  // of deps. E.g. require('@exmg/foo/bar') where the dep is '@exmg/foo'
  ...
  if (!sourceSpecifiedAsDep) {
    error(
      'Source not specified as a dependency within the component package.json. ' +
        `Dependencies specified are ${dependencies.join(', ')}`,
      source,
      srcFile,
      node,
    );
...
function error(msg, source, srcFile, node) {
  console.error(
    `🚫  ${srcFile}:${node.loc.start.line}:${node.loc.start.column} ` + `Invalid import source '${source}': ${msg}`,
  );
  exitCode += 1;
}
