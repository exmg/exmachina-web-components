 ...
 */
@customElement('exmg-markdown-editor-two')
export class EditorElement extends ExmgElement {
  ...
  renderHTML() {
    ...
    this.previewElement.innerHTML = `<div class="preview-body">${window.marked(this.markdown, opts)}</div>`;
    this.focus();
  ...
  setupEditor(): Editor {
    ...
    codeMirrorEditor.on('change', (editor: Editor) => {
      ...
      if (this.timer) {
        console.log('Change, Clearing timeout', this.timer);
        this.resetTogglePreviewTimer();
        this.setTogglePreviewTimer();
      }
    });
    ...
    codeMirrorEditor.on('focus', () => {
      if (!this.timer) {
        console.log('Focus, Setting timer', this.timer);
        this.setTogglePreviewTimer();
      }
    });
  ...
  togglePreview() {
    console.log('Toggle preview called', this.timer);
    this.timer = undefined;
    if (!this.editorElement || !this.previewElement) return;
    if (this.editorElement.classList.contains('visible')) {
      this.editorElement.classList.remove('visible');
      this.previewElement.classList.add('visible');
      this.previewVisible = true;
      console.log('Preview is visible', this.timer);
    } else {
      this.editorElement.classList.add('visible');
      this.previewElement.classList.remove('visible');
      this.codeMirrorEditor?.focus();
      this.previewVisible = false;
      console.log('Editor is visible', this.timer);
    }
  }

  setTogglePreviewTimer() {
    this.timer = setTimeout(() => this.togglePreview(), 5000);
  }

  resetTogglePreviewTimer() {
    ...
    return html`
      <div id="toolbar" class="toolbar">
        ...
        <div class=${classMap(classes)}>
          ${this.showHelperLabel ? html` <div>${this.previewVisible ? 'PREVIEW' : 'EDITOR'}</div> ` : ''}
        </div>
      </div>
      <div class="container" style="height: ${this.height && !this.fullscreen ? `${this.height}px` : 'inherit'};">
        <div id="editor"></div>
        <div id="preview" class="visible" @click=${this.togglePreview}></div>
      </div>
    `;
  }
}
